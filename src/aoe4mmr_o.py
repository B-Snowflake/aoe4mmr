#!/usr/bin/env python3 
# Author: B_Snowflake 
# Date: 2026/5/25 03:35:59 
# LastEditTime: 2026/5/25 03:35:59#!/usr/bin/env python3 
# Author: B_Snowflake 
# Date: 2026/5/25 00:25:56 
# LastEditTime: 2026/5/25 00:25:56#!/usr/bin/python3
# Author: B_Snowflake
# Date: 2025/4/1
...
class Aoe4mmr(QMainWindow):
    ...
    def __init__(self, pid_path):
        ...
        # 初始化数据库
        self.user_database_version = '1.0.7'
        ...
        self.last_game_id, self.last_game_data, self.temp_html, self.query_rs = None, None, None, {}
        self.new_version, self.release_url, self.only_databse_update = None, None, None
        # 初始化gui界面基本配置
        self.resize(965, 190)
        ...
        # 初始化数据线程
        self.data = data.Data(self, self.now_availiable_id, databasepath, logpath, self.map_dic, self.app_version)
        ...
        self.new_version_signal.connect(self.new_version_prompt)
        # 初始化子线程
    ...
    def new_version_prompt(self):
        if not hasattr(self, 'newversionwindow'):
            self.newversionwindow = SubWindow(main_window=self, main_widget=self.uiwidget, objectName='newversionwidget')
            self.newversionwindow.setWindowIcon(self.icon)
            self.newversionwindow.setWindowTitle('新版本可用')
            self.newversionwindow.resize(790, 150)
            self.newversionwidget = QWidget(parent=self.newversionwindow)
            self.newversionwindow.setCentralWidget(self.newversionwidget)
            self.newversionwidget_layout = QVBoxLayout()
            self.newversionwidget_layout.setContentsMargins(40, 30, 40, 30)
            self.newversionwidget.setLayout(self.newversionwidget_layout)
            text = '发现新版本，是否前往下载？'
            self.update_label = QLabel(text=text, parent=self.newversionwidget)
            self.newversionwidget_layout.addWidget(self.update_label, alignment=Qt.AlignCenter)
            self.yes_button = QPushButton(text='是', parent=self.newversionwidget)
            self.yes_button.clicked.connect(self.update_app)
            self.no_button = QPushButton(text='否', parent=self.newversionwidget)
            self.no_button.clicked.connect(self.cancle_update_app)
            self.yes_button.setMaximumHeight(30)
            self.yes_button.setMinimumWidth(200)
            self.no_button.setMaximumHeight(30)
            self.no_button.setMinimumWidth(200)
            self.button_layout = QHBoxLayout()
            self.button_layout.addStretch(1)
            self.button_layout.addWidget(self.yes_button)
            self.button_layout.addWidget(self.no_button)
            self.button_layout.addStretch(1)
            self.newversionwidget_layout.addLayout(self.button_layout)
            self.newversionwindow.show()
            self.newversionwindow.activateWindow()
        else:
            self.newversionwindow.show()
            self.newversionwindow.activateWindow()

    def cancle_update_app(self):
        # 不升级app时，关闭窗口。
        self.newversionwindow.hide()
        self.newversionwindow.deleteLater()

    def update_app(self):
        self.newversionwindow.hide()
        self.newversionwindow.deleteLater()
        if self.only_databse_update:
            download_thread = threading.Thread(target=self.download_db, daemon=True)
            download_thread.start()
        else:
            QDesktopServices.openUrl(QUrl(self.release_url))

    def download_db(self, retry_time=0):
        if retry_time > 2:
            return
        url = 'https://raw.githubusercontent.com/B-Snowflake/aoe4mmr/master/resources/~.pck'
        try:
            resp = requests.get(url, timeout=1000)  # 设定超时时间
            resp.raise_for_status()
            with open("resources/~.pck", "wb") as f:
                f.write(resp.content)
            self.get_all_data()
            self.data.app_version = self.app_version
        except requests.exceptions.Timeout:
            print(f"download timeout, retry {retry_time + 1}/3")
            retry_time = retry_time + 1
            self.download_db(retry_time)
    ...
    def get_all_data(self):
        ...
        self.html_data = data_cur.fetchone()[0]  # noqa
        app_version_rs = data_cur.execute('select version from version limit 1', )
        self.app_version = app_version_rs.fetchone()[0]
        data_conn.close()
        del data_conn
        resources_rc.qCleanupResources()

    def create_temp_html(self):
        if self.temp_html is None:
    ...
    def initilize_database(self):
        ...
        try:
            ...
            version = None
        if version != self.user_database_version:
            try:
            ...
            self.cur.execute("insert into version(version) values(?)", (self.user_database_version,))
            self.conn.commit()
    ...
    def center_window(win, is_top=True, is_right=False):
        ...
        new_x = (screen_geo.width() - win.width()) // 2
    ...
    #     if self.show_apm == 1:
    #         self.start_check_threads()
    #     self.apmwindowwidget = SubWindow(main_window=self, main_widget=self.uiwidget, postion_func=self.save_window_position,
    ...
    def on_load_finished(self):
        ...
        self.web_view_timer.start(500)  # 每0.5秒更新一次
    ...
    def setwindow(self):
        # 初始化设置窗口
        self.setwindowwidget = SubWindow(main_window=self, main_widget=self.uiwidget, postion_func=self.save_window_position)
    ...
    def set_profile_id(self):
        ...
        self.setprofileidwidget = SubWindow(main_window=self, main_widget=self.uiwidget, postion_func=self.save_window_position,
                                            on_focus=on_focus, objectName="setprofileidwidget")
...
class SubWindow(QMainWindow):
    ...
    def moveEvent(self, event):
        ...
        if self.objectName() == 'setprofileidwidget':
            self.main_window.game_id.suggestion_list.move(self.mapToGlobal(self.main_window.game_id.geometry().bottomLeft()))
            self.main_window.game_id.suggestion_list.setFixedWidth(self.main_window.game_id.width())
            if self.main_window.game_id.suggestion_list.count() > 0:
                self.main_window.game_id.suggestion_list.show()
    ...
    def mouseReleaseEvent(self, event: QEvent):
        if event.button() == Qt.MouseButton.LeftButton and self.testAttribute(Qt.WA_TranslucentBackground):
            ...
            self.resize_dragging = False
            self.setCursor(Qt.CursorShape.ArrowCursor)
        if self.postion_func is not None:
...
class SearchCompleter(QLineEdit):
    ...
    def set_suggestions_list(self, suggestions_dic):
        if all(k not in suggestions_dic for k in ("正在检索ID，请稍候...", "没有检索到相关ID")):
